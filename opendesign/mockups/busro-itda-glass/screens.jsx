    ...
    <main className="screen content-screen journey-screen">
      ...
      <section className="leg-timeline">
        {steps.map((step, index) => {
          ...
          return (
          <article key={`${step.kind || "step"}-${step.route_id || "none"}-${index}`} className={index === 0 ? "current" : ""}>
            <div className="leg-rail"><span>{index + 1}</span><i /></div>
            <div className="leg-card"><div><p><span className="line-chip blue">{stepLabel}</span>{step.kind === "transfer" ? "정류장 간 이동" : "버스 이동"}</p><h3>{stepFrom.node_name || stepFrom.node_id || "DATA_GAP"} → {stepTo.node_name || stepTo.node_id || "DATA_GAP"}</h3><small>{typeof step.distance_m === "number" ? `${Math.round(step.distance_m)}m · ` : ""}{stepEvidence.type || "근거 DATA_GAP"}{stepEvidence.source ? ` · ${stepEvidence.source}` : ""}</small></div></div>
          </article>
          );
        })}
      </section>
      ...
      <InlineNotice tone={status === "READY" ? "success" : "warning"} icon={status === "READY" ? "shield-check" : "warning-circle"} title={status}>
        {reasons.length > 0 ? reasons.join(" · ") : "추가 결측 사유 없음"}

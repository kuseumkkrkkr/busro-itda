...
def _cache_control(path: str) -> str:
    if path in {"/api/network/status", "/api/network/cities", "/api/network/stops", "/api/network/routes", "/api/sources", "/api/sources/discover", "/api/journeys/research"}:
    ...
    try:
        if method == "GET":
            handlers = {
                ...
                "/api/journeys/research": lambda: service.journey_research(query_data),

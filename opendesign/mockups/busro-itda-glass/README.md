...
키를 이미 숨김 입력으로 보유한 로컬 서비스가 다른 포트에서 실행 중이면, 새 웹 프로세스는 키를 다시 받지 않고 제한된 loopback bridge로 재사용할 수 있습니다. 두 프로세스가 같은 이력·카탈로그 SQLite를 사용한다고 운영자가 명시한 경우에만 수집·노선 적재 쓰기를 허용하며, 시작 시 카운트와 실제 쓰기 결과를 다시 확인합니다. 상세 명령은 `service/README.md`를 따릅니다.
